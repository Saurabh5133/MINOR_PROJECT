...
class SQLiteEngine:
    ...
    def estimate_cost(self, query: str, schema: dict) -> dict:
        ...
        total = scan_cost + join_cost + sort_cost + agg_cost + subq_cost
        total = max(1.0, total)
    ...
    def _col_factor(self, query: str, rc_list: list, alias_info: dict) -> float:
        ...
        # Skip pure aggregate queries — no projection savings
        # e.g. SELECT COUNT(*), SUM(x) — but NOT SELECT col, COUNT(*) mixed
        non_agg_cols = re.sub(
            r'\b(?:COUNT|SUM|AVG|MIN|MAX)\s*\([^)]*\)(?:\s+AS\s+\w+)?',
            '', select_text, flags=re.IGNORECASE
        ).strip().strip(',').strip()
        if not non_agg_cols:
            # Purely aggregate — no projection savings
            return 1.0
    ...
    def _selectivity(self, query: str) -> float:
        ...
        return round(max(0.001, min(1.0, self._parse_conditions(clause))), 6)
    ...
    def _split_top(self, text: str, keyword: str) -> list:
        ...
        while i < len(text):
            ...
            elif depth == 0 and tu[i:i+len(kw)] == kw:
