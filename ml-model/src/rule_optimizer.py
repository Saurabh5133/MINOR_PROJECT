...
class RuleOptimizer:
    def __init__(self):
        self.rules = [
            ...
            self._rule_add_limit,                 # ORDER BY without LIMIT
        ]
    ...
    def _rule_where_condition_reorder(self, query: str, schema: dict = None) -> Optional[tuple]:
        ...
        new_query = (query[:where_m.start(1)] +
                     new_where +
                     query[where_m.start(1) + len(where_body):])
    ...
    def _rule_predicate_before_join(self, query: str, schema: dict = None) -> Optional[tuple]:
        ...
        from_alias = from_m.group(2) or from_m.group(1)
